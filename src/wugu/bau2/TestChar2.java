...
public class TestChar2 extends AdvGeometry
{
	...
	public TestChar2(String name)
	{
		super(name, new Vector2f(0.25f, 0.25f)/*Vector2f.UNIT_XY*/);
	}
	...
	protected void setup()
	{
		...
		l1(-1);
		l1(1);
		tc2a = new TC2a("Unshaded", texScale, boneList, zk0a, zk0b, k3a0[0], k3a0[1]);
		tc2a.setup();
		tc2a.finish();
		a1(-1);
		a1(1);
	...
	private void k1()
	{
		...
		bau.cylinderTex2DP(null, false, zk0a, zk0b, zk1a, zk1b);
		bau.cylinderTex2DP(null, false, zk1a, zk1b, zk2a, zk2b);
		bau.cylinderTex2DP(null, false, zk2a, zk2b, zk3a, zk3b);
		bau.indexQ(true, zk3b, zk3b.sCl(1), zk4b, zk4b.sCl(1));
		bau.indexQ(true, zk3b.sCl(-2), zk3b.sCl(-1), zk4b.sCl(-2), zk4b.sCl(-1));
	}
	...
	private void l1(int xm)
	{
		...
		if(k3a0 == null)
			k3a0 = new DualP2[2];
		k3a0[(xm + 1) / 2] = k3a;
		bau.cylinderTexSpin(null, true, 8, k1a, k2a);
		bau.cylinderTexSpin(null, true, 8, k2a, k3a);
	...
	private void s1(int xm, AutoNumBone l3)
	{
		...
		bau.cylinderTex2DP(null, xm > 0, s1a, s1b, s2a, s2b);
		bau.cylinderTex2DP(0b1111100, xm > 0, s2a1, s2b1, s2ca, s2cb);
		bau.cylinderTex2DP(0b1111100, xm > 0, s2ca, s2cb, s3ca, s3cb);
		bau.cylinderTex2DP(0b1111100, xm > 0, s3ca, s3cb, sbca, sbcb);
		bau.flatPoly(xm < 0, new Position2[]{sbcb.sCl(2), sbcb.sCl(3), sbca.sCl(3), sbca, sbcb, sbcb.sCl(1)},
		...
		bau.indexQ(xm > 0, s4ce, s4ca, sfce, sfca);
		bau.indexQ(xm > 0, s4ca, s4cb, sfca, sfcb);
		bau.indexQ(xm > 0, s4cb, s4cd, sfcb, sfcd);
		bau.indexQ(xm > 0, s4cd, s4cc, sfcd, sfcc);
		bau.indexQ(xm > 0, s4cc, s4ca.sCl(2), sfcc, sfca.sCl(2));
		bau.indexQ(xm > 0, s4ca.sCl(2), s4ceC, sfca.sCl(2), sfceC);
		bau.flatPoly(xm > 0, new Position2[]{sfce, sfca, sfcb, sfcd, sfcc, sfca.sCl(2), sfceC},
		...
		bau.indexQ(xm < 0, s4ce, s2bC23, sfce, sbdb);
		bau.indexQ(xm > 0, s4ceC, s2bC13, sfceC, sbdbC);
		bau.flatPoly(xm < 0, new Position2[]{sfce, sbdb, sbdbC, sfceC},
				new Vector3f(-0.05f * xm, -1, 0), new Vector2f(2.7f, 0.4f), new Vector2f(2.5f, 0.4f),
				new Vector2f(2.5f, 0.6f), new Vector2f(2.7f, 0.6f));
		bau.flatPoly(xm > 0, new Position2[]{sbcb.sCl(2), sbdb, sbdbC, sbcb.sCl(1)},
		...
		DualP2 s2b2 = new DualP2(new Vector3f(-KS2 * xm, 0, -KS1), new Vector3f(-0.1f * xm, 1, -0.2f),
				new Vector2f(2.1f, 0.8f), l4);
		bau.indexPoly(xm < 0, s4cd, s4cc, s5a.sCl(3), s5a, s4cb);
		bau.indexQ(xm < 0, s5a, s5a.sCl(3), s6a, s6a.sCl(3));
		bau.indexPoly(xm < 0, s4cb, s5a, s4ca);
		bau.indexPoly(xm > 0, s4cc, s5a.sCl(3), s4ca.sCl(2));
		bau.indexPoly(xm < 0, s4ca, s5a, s6a, s4ce);
		bau.indexPoly(xm > 0, s4ca.sCl(2), s5a.sCl(3), s6a.sCl(3), s4ceC);
		bau.indexPoly(xm < 0, s4ce, s6a, s2bC22, s2bC23);
		bau.indexPoly(xm > 0, s4ceC, s6a.sCl(3), s2bC12, s2bC13);
		bau.indexQ(xm < 0, s6a, s6a.sCl(3), s2a2.sCl(2), s2a2.sCl(1));
		bau.indexPoly(xm < 0, s2bC21, s2bC22, s6a);
		bau.indexPoly(xm > 0, s2bC11, s2bC12, s6a.sCl(3));
		bau.indexPoly(xm < 0, s2a2.sCl(2), s2b2.sCl(2), s6a);
		bau.indexPoly(xm > 0, s2a2.sCl(1), s2b2.sCl(1), s6a.sCl(3));
	}
	...
	private void a1(int xm)
	{
		...
		bau.indexPoly(false, mkt1, mkt1C, k1a.sCl(1), k1b.sCl(1), k1b, k1a);
		bau.indexQ(true, mkt1, mkt1C, mkt2, mkt2C);
		bau.indexQ(true, mkt1, mkt2, k1a, k1a.sCl(-1));
		bau.indexQ(false, mkt1C, mkt2C, k1a.sCl(1), k1a.sCl(2));
		bau.indexPoly(true, mkt2, mkt2C, zk4a.sCl(-1 - xm), zk4a.sCl(xm), zk4b.sCl(xm));
		bau.indexPoly(true, mkt2C, zk4b.sCl(-1 - xm), zk4a.sCl(-1 - xm));

		bau.indexQ(false, zk3a.sCl(-xm - 1), zk3a.sCl(xm), k1b.sCl(-2), k1b.sCl(-1));
		bau.indexQ(true, zk3b.sCl(xm), zk3a.sCl(xm), k1a.sCl(-1), k1b.sCl(-1));
		bau.indexQ(true, zk3a.sCl(-1 - xm), zk3b.sCl(-1 - xm), k1b.sCl(-2), k1a.sCl(-2));
		bau.indexQ(false, zk3b.sCl(xm), zk4b.sCl(xm), k1a.sCl(-1), mkt2);
		bau.indexQ(true, zk3b.sCl(-1 - xm), zk4b.sCl(-1 - xm), k1a.sCl(-2), mkt2C);

		bau.cylinderTex2DP(null, true, k1a, k1b, k2a, k2b);
		bau.cylinderTex2DP(null, true, k2a, k2b, k2c, k2d);
		bau.cylinderTex2DP(null, true, k2c, k2d, k3c, k3d);
		bau.cylinderTex2DP(null, true, k3c, k3d, k3ch, k3dh);
	...
	private void h1(int xm, AutoNumBone a3)
	{
		...
		for(int i = 0; i < 5; i++)
		{
			...
			for(int j = 1; j < (i == 0 ? 2 : 3); j++)
			{
				...
				bau.cylinderTex1DP(null, xm > 0, lastf, newf);
				lastf = newf;
			}
			int ab = 5 * (i == 0 ? 2 : 3) + i;
			...
			bau.indexQ(xm < 0, end1, end1.sCl(1), end2.sCl(-1), end2.sCl(2));
			bau.indexQ(xm < 0, lastf, lastf.sCl(1), end1, end1.sCl(1));
			bau.indexQ(xm > 0, lastf, lastf.sCl(-1), end1, end2.sCl(-1));
			bau.indexQ(xm > 0, lastf.sCl(2), lastf.sCl(1), end2.sCl(2), end1.sCl(1));
			bau.indexPoly(xm > 0, lastf.sCl(-1), end4.sCl(-2), end4.sCl(-1), end2.sCl(-1));
			bau.indexPoly(xm > 0, lastf.sCl(-2), end2.sCl(-2), end4.sCl(-2), lastf.sCl(-1));
			...
			bau.indexPoly(xm < 0, end3a.sCl(3), end3a.sCl(2), end2a.sCl(2),
		...
		for(int i = 0; i < 4; i++)
			bau.indexPoly(xm > 0, d1[i].sCl(1), d1[i].sCl(-2), d1[i + 1].sCl(-1), d1[i + 1]);
		bau.indexPoly(xm < 0, khc.sCl(1), khc.sCl(-2), d1[0].sCl(-2), d1[1].sCl(-1));
		bau.indexPoly(xm > 0, khc.sCl(-2), d1[0].sCl(-2), d1[0].sCl(-1));
		bau.indexPoly(xm > 0, khc.sCl(-2), d1[0].sCl(-1), d1[0]);

		bau.indexQ(xm > 0, khc.sCl(1), khd.sCl(1), d1[1].sCl(-1), d1[1].sCl(-2));
		bau.indexPoly(xm < 0, khd.sCl(1), d1[1].sCl(-2), d1[2].sCl(-1));
		bau.indexPoly(xm < 0, khd.sCl(1), d1[2].sCl(-1), d1[2].sCl(-2));
		bau.indexQ(xm > 0, khd.sCl(1), khd, d1[2].sCl(-2), d1[3].sCl(-1));
		bau.indexPoly(xm < 0, khd, d1[3].sCl(-1), d1[3].sCl(-2));
		bau.indexPoly(xm < 0, khd, d1[3].sCl(-2), d1[4].sCl(-1));
		bau.indexQ(xm > 0, khd, khc, d1[4].sCl(-1), d1[4].sCl(-2));

		bau.indexQ(xm > 0, khc, khc.sCl(-1), d1[4].sCl(-2), d1[4].sCl(1));
		bau.indexQ(xm > 0, khc.sCl(-1), khd.sCl(-1), d1[4].sCl(1), d1[4]);
		bau.indexPoly(xm < 0, khd.sCl(-1), d1[4], d1[3].sCl(1), d1[3], d1[2].sCl(1),
				d1[2], d1[1].sCl(1), d1[1], d1[0].sCl(1));
		bau.indexPoly(xm > 0, khd.sCl(-1), khd.sCl(-2), d1[0].sCl(1));
		bau.indexQ(xm > 0, khd.sCl(-2), khc.sCl(-2), d1[0].sCl(1), d1[0]);
	}
	...
	private void z1()
	{
		...
		for(int i = 0; i < 8; i++)
		{
			...
			bau.indexPoly(true, a1);
		}

		bau.cylinderTexSpin(0b1111111111000011, false, 16, z1a, z2a);
		tc2b0 = bau.indexes.size();
		bau.cylinderTexSpin(0b1111111111000011, false, 16, z2a, z3a);
		bau.cylinderTexSpin(0b1111111111000011, false, 16, z3a, z4a);
		bau.abdeckX(false, 16, z4b, z4a);
		tc2b1 = bau.indexes.size();
		bau.indexPoly(true, z3a.sCl(edgm + 2), z4a.sCl(edgm + 2), z4a.sCl(edgm + 3));
		bau.indexPoly(true, z4a.sCl(edgm + 4), z4a.sCl(edgm + 5), z3a.sCl(edgm + 6),
				z3a.sCl(edgm + 2), z4a.sCl(edgm + 3));
		bau.indexPoly(true, z4a.sCl(edgm + 5), z4a.sCl(edgm + 6), z3a.sCl(edgm + 6));
		bau.indexPoly(false, z2a.sCl(edgm + 2), z1a.sCl(edgm + 2), z1a.sCl(edgm + 3));
		bau.indexPoly(false, z1a.sCl(edgm + 4), z1a.sCl(edgm + 5), z2a.sCl(edgm + 6),
				z2a.sCl(edgm + 2), z1a.sCl(edgm + 3));
		bau.indexPoly(false, z1a.sCl(edgm + 5), z1a.sCl(edgm + 6), z2a.sCl(edgm + 6));
		//bau.indexQ(true, z3a.sCl(edgm + 2), z3a.sCl(edgm + 6), z2a.sCl(edgm + 2), z2a.sCl(edgm + 6));
	...
	private void z2(Position2 lo, Position2 ro, Position2 lu, Position2 ru)
	{
		...
		bau.indexPoly(true, z2a1, z2a2.sCl(1), lo, ro, z2a2);
		bau.indexPoly(true, z2a5, z2a4, ru, lu, z2a4.sCl(1));
		bau.indexPoly(false, z2a3, z2a4, ru, ro, z2a2);
		bau.indexPoly(true, z2a3.sCl(1), z2a4.sCl(1), lu, lo, z2a2.sCl(1));
		//bau.indexQ(true, lo, ro, z2a1.sCl(1), z2a1.sCl(0));
	}
}