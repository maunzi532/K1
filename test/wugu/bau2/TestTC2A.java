...
public class TestTC2A extends SimpleApplication
{
	TestChar2A tc;
	...
	public void simpleInitApp()
	{
		...
		tc = new TestChar2A();
		tc.init("Wugu");
		tc.attachToNodes(rootNode);
		tc.mainGeo.geom.setMaterial(MaterialLager.wand1);
		...
		tc.mainGeo.geom.getMaterial().setTexture("DiffuseMap", t);
		tc.mainGeo.geom.getMaterial().setBoolean("UseMaterialColors", false);
		...
		//tc.mainGeo.geom.getMaterial().getAdditionalRenderState().setWireframe(true);
		//tc.geom.getMaterial().getAdditionalRenderState().setFaceCullMode(RenderState.FaceCullMode.Off);
		tc.mainGeo.controlSetup();
		...
		qg.getMaterial().setTexture("ColorMap", tc.mainGeo.bau.texfill(2560, 2560, new Vector2f(0.25f, 0.25f)));
	...
	public void simpleUpdate(float tpf)
	{
		...
		tc.mainGeo.sk.getBone(0 + 43).setUserTransforms(Vector3f.ZERO, rotation, Vector3f.UNIT_XYZ);
		tc.mainGeo.sk.updateWorldVectors();*/
		/*tc.tc2b.castPhysics();
